...
mod tests {
    ...
    fn test_waiting_to_running_to_waiting_fires_twice() {
        let (mut watcher, dir) = setup_event_watcher(
            r#"{"worktrees":[{"id":"w1","branch":"swarm/bounce","agent":{"pane_id":"%1"},"created_at":"2026-02-26T10:00:00-08:00","summary":"Bouncy task"}]}"#,
        );
        ...
        assert!(
            ...
            "running transition should not fire AgentWaiting"
        );
    ...
    fn test_pr_already_set_at_init_no_pr_opened() {
        // Worker already has pr_url at startup, AND the PR was already notified
        // in a previous daemon run. PrOpened should NOT be emitted.
        let mut file = NamedTempFile::new().unwrap();
        let hive_dir = tempfile::tempdir().unwrap();

        // Pre-populate pr_notified.json as if previous daemon already sent PrOpened.
        let notified_path = hive_dir.path().join("pr_notified.json");
        std::fs::write(
            &notified_path,
        ...
        write_state(
            &mut file,
            r#"{"worktrees":[{"id":"w1","branch":"swarm/old-pr","agent":{"pane_id":"%1"},"created_at":"2026-02-26T10:00:00-08:00","pr":{"url":"https://github.com/ApiariTools/hive/pull/50","title":"Old feature"}}]}"#,
        );
        ...
        // Second poll — PR is already in pr_notified, should NOT fire PrOpened.
        let notes = watcher.poll();
        let pr_notes: Vec<_> = notes
            .iter()
            .filter(|n| matches!(n, SwarmNotification::PrOpened { .. }))
        ...
        assert!(
            pr_notes.is_empty(),
            "should NOT emit PrOpened for PR already in pr_notified.json at startup"
        );
    }
    ...
    fn test_multiple_workers_appear_simultaneously() {
        let (mut watcher, dir) = setup_event_watcher(
            r#"{"worktrees":[
                {"id":"w1","branch":"swarm/feat-a","agent":{"pane_id":"%1"},"created_at":"2026-02-26T10:00:00-08:00","summary":"Feature A"},
                {"id":"w2","branch":"swarm/feat-b","agent":{"pane_id":"%2"},"created_at":"2026-02-26T10:00:00-08:00","summary":"Feature B"}
            ]}"#,
        );
        ...
        assert_eq!(
            spawned.len(),
            2,
            "both workers should emit AgentSpawned in the same poll"
        );

        let wt_ids: HashSet<&str> = spawned.iter().map(|n| n.worktree_id()).collect();
        assert!(wt_ids.contains("w1"), "w1 should be in spawned set");
        assert!(wt_ids.contains("w2"), "w2 should be in spawned set");
    }
}
