...
mod tests {
    ...
    fn test_waiting_to_running_to_waiting_fires_twice() {
        ...
        // Second transition: waiting → running (user sends input, agent resumes).
        append_event(
            &dir,
            r#"{"event":"phase_changed","worktree":"w1","from":"waiting","to":"running","timestamp":"2026-02-26T10:06:00-08:00"}"#,
        );
        let notes = watcher.poll();
        // running from waiting does NOT match the guard (only from creating/starting).
        assert!(
            notes
                .iter()
    ...
    fn test_pr_already_set_at_init_no_pr_opened() {
        // Worker already has a pr_url when the watcher initializes,
        // BUT no hive_dir is set (no pr_notified persistence). The init
        // code only tracks prs_at_init when hive_dir is set, so without
        // it PrOpened should NOT be emitted on the second poll.
        let mut file = NamedTempFile::new().unwrap();
        write_state(
            &mut file,
            r#"{"worktrees":[{"id":"w1","branch":"swarm/existing-pr","agent":{"pane_id":"%1"},"created_at":"2026-02-26T10:00:00-08:00","pr":{"url":"https://github.com/ex/pull/99","title":"Already there"}}]}"#,
        );

        let mut watcher = SwarmWatcher::new(file.path().to_path_buf());
        // Do NOT set hive_dir — simulates no persistence.
        watcher.poll(); // Initialize

        // Verify prs_at_init is empty (no hive_dir → no tracking).
        assert!(
            watcher.prs_at_init.is_empty(),
            "without hive_dir, prs_at_init should be empty"
        );

        // Second poll — no events, no PrOpened should fire.
        let notes = watcher.poll();
        let pr_notes: Vec<_> = notes
            .iter()
            .filter(|n| matches!(n, SwarmNotification::PrOpened { .. }))
        ...
        assert!(
            pr_notes.is_empty(),
            "PrOpened should NOT be emitted when pr_url is already set at init without hive_dir"
        );
    }
    ...
    fn test_multiple_workers_appear_simultaneously() {
        ...
        assert_eq!(
            spawned.len(),
            2,
            "two simultaneous workers should produce two AgentSpawned events"
        );

        let ids: Vec<&str> = spawned.iter().map(|n| n.worktree_id()).collect();
        assert!(ids.contains(&"w1"), "w1 should be in spawned notifications");
        assert!(ids.contains(&"w2"), "w2 should be in spawned notifications");
    }
}
